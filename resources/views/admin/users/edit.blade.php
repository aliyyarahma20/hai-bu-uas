...
@section('title', 'Edit Pengguna - Hai Bu')
...
<div id="menu-content" class="flex flex-col w-full pb-[30px]">
    ...
    <div class="flex flex-col gap-10 px-5 mt-5">
        <div class="breadcrumb flex items-center gap-[30px]">
            ...
            <a href="{{ route('dashboard.user.index')}}" class="text-[#7F8190] last:text-[#0A090B] last:font-semibold">Pengguna</a>
            <span class="text-[#7F8190] last:text-[#0A090B]">/</span>
            <a href="{{ route('dashboard.user.edit', $user)}}" class="text-[#7F8190] last:text-[#0A090B] last:font-semibold ">Detail pengguna</a>
        </div>
    </div>
    <div class="header flex flex-col gap-1 px-5 mt-5">
        <h1 class="font-extrabold text-[30px] leading-[45px]">Detail Pengguna</h1>
    </div>
        @if ($errors->any())
            <ul>
        ...
        @endif
    <form method="POST" enctype="multipart/form-data" action="{{ route('dashboard.user.update', $user)}}" class="flex flex-col gap-[30px] w-[500px] mx-[70px] mt-10">
        @csrf
        @method('PUT')
        ...
        <div class="flex flex-col gap-[10px]">
            ...
            <div class="flex items-center w-[500px] h-[52px] p-[14px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                ...
                <input value="{{$user->name}}" type="text" class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal w-full outline-none"  name="name" required>
            </div>
        </div>
        <div class="flex flex-col gap-[10px]">
