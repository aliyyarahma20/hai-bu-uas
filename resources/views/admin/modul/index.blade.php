        ...
        <div id="sidebar" class="w-[270px] flex flex-col shrink-0 min-h-screen justify-between p-[30px] border-r border-[#EEEEEE] bg-[#FBFBFB]">
            <div class="w-full flex flex-col gap-[30px]">
                ...
                <ul class="flex flex-col gap-3">
                    ...
                    <li>
                        <a href="" class="p-[10px_16px] flex items-center gap-[14px] rounded-full h-11 bg-[#91AC8F] transition-all duration-300 hover:bg-[#66785F]">
                            <div>
                                <img src="{{asset('images/icons/note-favorite.svg')}}" alt="icon">
                            </div>
                            <p class="font-semibold text-white transition-all duration-300 hover:text-white">Modul Bahasa</p>
                        </a>
                    </li>
                    <li>
                        <a href="" class="p-[10px_16px] flex items-center gap-[14px] rounded-full h-11 transition-all duration-300 hover:bg-[#66785F]">
                            ...
                            <p class="font-semibold transition-all duration-300 hover:text-white">Pengguna</p>
                        </a>
                    </li>
                    <li>
                        <a href="" class="p-[10px_16px] flex items-center gap-[14px] rounded-full h-11 transition-all duration-300 hover:bg-[#66785F]">
                ...
                <ul class="flex flex-col gap-3">s
                    <li>
                    ...
                    <li>
                        <a href="signin.html" class="p-[10px_16px] flex items-center gap-[14px] rounded-full h-11 transition-all duration-300 hover:bg-[#66785F]">
                            <div>
        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <div class="flex flex-col px-5 mt-5">
                <div class="w-full flex justify-between items-center">
                    ...
                    <a href="{{route('dashboard.module-bahasa.create')}}" class="h-[52px] p-[14px_20px] bg-[#91AC8F] rounded-full font-bold text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D]">Add New Course</a>
                </div>
            </div>
                        ...
                        <div class="flex shrink-0 w-[300px]">
                            <div class="flex items-center gap-4">
                                <div class="w-16 h-16 flex shrink-0 overflow-hidden rounded-full">
                                    <img src="{{Storage::url($modul->cover)}}" class="object-cover" alt="thumbnail">
                                </div> 
                                <div class="flex flex-col gap-[2px]">
                                    <p class="font-bold text-lg">{{$modul->nama}}</p>
                                    <p class="text-[#7F8190]">Beginners</p>
                                </div>
                            </div>
                        </div>
                        <div class="flex shrink-0 w-[150px] items-center justify-center">
                            <p class="font-semibold">{{\Carbon\Carbon::parse($modul->created_at)->format('F j, Y')}}</p>
                        </div>
                        @if($modul->category->name == "Bahasa Sunda" or $modul->category->name == "Bahasa Minangkabau")
                            <div class="flex shrink-0 w-[170px] items-center justify-center">
                                <p class="p-[8px_16px] rounded-full bg-[#FFF2E6] font-bold text-sm text-[#F6770B]">{{$modul->category->name}}</p>
                            </div>
                        @elseif($modul->category->name == "Bahasa Jawa")
                            <div class="flex shrink-0 w-[170px] items-center justify-center">
                        ...
                        <div class="flex shrink-0 w-[120px] items-center">
                            <div class="relative h-[41px]">
                                <div class="menu-dropdown w-[120px] max-h-[41px] overflow-hidden absolute top-0 p-[10px_16px] bg-white flex flex-col gap-3 border border-[#EEEEEE] transition-all duration-300 hover:shadow-[0_10px_16px_0_#0A090B0D] rounded-[18px]">
                                    ...
                                    <a href="{{route('dashboard.module-bahasa.show', $modul)}}" class="flex items-center justify-between font-bold text-sm w-full">
                                        Manage
                                    </a>
                                    <a href="course-students.html" class="flex items-center justify-between font-bold text-sm w-full">
                                        Students
                                    </a>
                                    <a href="{{route('dashboard.module-bahasa.edit', $modul)}}" class="flex items-center justify-between font-bold text-sm w-full">
                                        Edit Course
                                    </a>
                                    <form method="POST" action="{{route('dashboard.module-bahasa.destroy', $modul)}}">
                                        @csrf
                                        @method('DELETE')
